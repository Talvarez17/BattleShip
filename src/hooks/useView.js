...
const useView = (viewOnly = true, socket, opponentId) => {
    const reducers = {
        ...
        [OPPONENT_LEFT]({ messages }) {
            return { ...initialState(), messages, haveSendInitialMsg: true, };
        },
        ...
        [SELECT_TILE](state, { coordinate: selectedCoordinate }) {
            ...
            const newChosenTiles = isSelected
                ? chosenTiles.filter(
                    (coordinate) => !checkIfSameCoordinate(coordinate, selectedCoordinate)) : chosenTiles.concat([selectedCoordinate]);
        ...
        [CONFIRM_TILES](state) {
            ...
            return { ...state, myShips: newMyShips, shipTilesState: newShipTilesState, chosenTiles: [] };
        ...
        [SHOT](state, { coordinate }) {
            ...
            return { ...state, opponentShipsShot: newOpponentShipsShot, gameState: newGameState };
        },
    ...
    const { gotInitialOpponent, opponent, haveSendInitialMsg, gameState, myShips, opponentShips, messages, shipTilesState, chosenTiles, opponentShipsShot, myShipsShot } = state;
    ...
    useEffect(() => {
        ...
        socket.on("opponent", ({ data, sender }) => {

            console.log("New opponent");
            if (sender === opponentId) {
                dispatch({ data, type: NEW_OPPONENT });
            }
        });
    ...
    useEffect(() => {
        socket.on("viewer-states", (states) => {
            if (states.state.opponent == opponentId) {
                dispatch({ type: SET_VIEWER_STATE, states });
            }
        })
    }, [viewOnly]);
    ...
    useEffect(() => {
        if (gotInitialOpponent) {
            ...
            const message = opponent ? haveSendInitialMsg ? MSG_HAVE_OPPONENT : INITIAL_MSG_HAVE_OPPONENT : haveSendInitialMsg ? MSG_NO_OPPONENT : INITIAL_MSG_NO_OPPONENT;
        ...
        switch (gameState) {
            ...
            case 3:
                ...
                if (opponentLastShot) {
                    ...
                    const justSinkShipName = findSinkShipNameOfCoordinate(myShips, opponentLastShot, myShipsShot);
                    if (justSinkShipName) {
            ...
            case 4:
                ...
                if (myLastShot) {
                    ...
                    const justSinkShipName = findSinkShipNameOfCoordinate(opponentShips, myLastShot, opponentShipsShot);
    ...
    const showMyOverlay = gameState === 5 ? MSG_WIN : gameState === 6 ? MSG_LOSE : null;
